...
#Region "Definations for Command Strings"
    ...
    Dim ReadDataCmd() As Byte = {&H44, &H1} 'D 1/2/3/4  CH1 200/CH2 200/CH1 FFT First 200/CH2 Next 200
...
#Region "Gain, Offset and Trigger Level, Vref"
    ...
    Dim Ch1_Offset_Scroll_Corrected, Ch2_Offset_Scroll_Corrected, Ch1_Trigger_Scroll_Corrected As Integer
...
#Region "Other variables"
    ...
    Dim Limit_Value As Integer 'for messagebox limit values
...
#Region "Main Form Load"
    Private Sub Scope_Main_Form_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Sample_Rate_ListBox.Text = "500kbps 2.0us/sample"
        Scale_Factor = 1
    End Sub
#End Region
...
#End Region
    'Scrollbar  Values
#Region "Scrollbar Values"
    Private Sub CH1_TrigLevel_VScrollBar_Scroll(ByVal sender As System.Object, ByVal e As System.Windows.Forms.ScrollEventArgs) Handles CH1_TrigLevel_VScrollBar.Scroll
        Get_Gain()
        Write_Scrollbar_Offsets()
    End Sub

    Private Sub CH2_Offset_VScrollBar_Scroll(ByVal sender As System.Object, ByVal e As System.Windows.Forms.ScrollEventArgs) Handles CH2_Offset_VScrollBar.Scroll
        Get_Gain()
        Write_Scrollbar_Offsets()
    End Sub

    Private Sub CH1_Offset_VScrollBar_Scroll(ByVal sender As System.Object, ByVal e As System.Windows.Forms.ScrollEventArgs) Handles CH1_Offset_VScrollBar.Scroll
        Get_Gain()
        Write_Scrollbar_Offsets()
    End Sub
#End Region
#Region "Setup Gain, Offset , Trigger Level & VREF"
    'Gain Values
    Private Sub Get_Gain()
        If CH1_Gain_RadioButton1.Checked = True Then '-12.5V to +12.5V
            Ch1_Gain = 1
        ElseIf CH1_Gain_RadioButton2.Checked = True Then '-6.25V to +6.25V
            Ch1_Gain = 2
        ElseIf CH1_Gain_RadioButton3.Checked = True Then '-2.5V to +2.5V
            Ch1_Gain = 5
        End If
        'Ch2
        ...
        End If
    End Sub
    'Write Raw Offsets in Scrollbars
    Private Sub Write_Scrollbar_Offsets()
        Dim temp As Integer
        'Brings to 0-1023 range
        Ch1_Offset_Scroll_Corrected = CH1_Offset_VScrollBar.Value * -1
        Ch2_Offset_Scroll_Corrected = CH2_Offset_VScrollBar.Value * -1
        Ch1_Trigger_Scroll_Corrected = CH1_TrigLevel_VScrollBar.Value * -1

        Ch1_Offset = Ch1_Offset_Scroll_Corrected 'raw values
        Ch2_Offset = Ch2_Offset_Scroll_Corrected
        Ch1_Trig = Ch1_Trigger_Scroll_Corrected

        'write raw values to screen
        'keep Offset unchanged ,Trig Depends on Gain
        CH1_Offset_TextBox.Text = Math.Round((Ch1_Offset * 12.5 / 512 - 12.5) * Scale_Factor, 2)
        CH2_Offset_TextBox.Text = Math.Round((Ch2_Offset * 12.5 / 512 - 12.5) * Scale_Factor, 2)

        temp = Ch1_Trig - 512

        If Ch1_Trig_RadioButton.Checked = True Then
            temp = (temp * Ch1_Gain) + 512
            CH1_Trig_TextBox.Text = Math.Round((Ch1_Trig * 12.5 / 512 - 12.5) * Scale_Factor, 2)
        ElseIf CH2_Trig_RadioButton.Checked = True Then
            temp = (temp * Ch2_Gain) + 512
            CH1_Trig_TextBox.Text = Math.Round((Ch1_Trig * 12.5 / 512 - 12.5) * Scale_Factor, 2)
        Else
            temp = 512
            CH1_Trig_TextBox.Text = "N/A"
        End If
        'Limiting the Trigger value 

        If (temp <= 0) Then
            temp = 0

        ElseIf (temp >= 1023) Then
            temp = 1023
        End If
    ...
    Private Sub Compute_Offset_Trigger()
        ...
        Scale_Factor = Math.Round(673 / Vref, 3) '673=1024 * (3.29/5) 
        'Scale_Factor = 1
        ...
        Offset_Gain_Factor = (1.25) ' no variation with / Scale_Factor !
        'Offset_Gain_Factor = (5 / 4) / Scale_Factor
        'These are the Zero Offset Values
        Ch1_Offset1 = (512 * Offset_Gain_Factor) / Ch1_Gain
        Ch2_Offset1 = (512 * Offset_Gain_Factor) / Ch2_Gain


        'Additional offset to shift the display by the manual offset values 1024 is the max value

        Ch1_Offset1 = Ch1_Offset1 + (Ch1_Offset_Scroll_Corrected - 512) * Offset_Gain_Factor
        Ch2_Offset1 = Ch2_Offset1 + (Ch2_Offset_Scroll_Corrected - 512) * Offset_Gain_Factor
        ...
        If Ch1_Offset1 <= 0 Then
            Ch1_Offset1 = 0
            '.Show("Ch1 Offset Limited to Minimum")
        End If

        If Ch1_Offset1 >= 1024 Then
            Ch1_Offset1 = 1023
            'MessageBox.Show("Ch1 Offset Limited to Maximum")
        End If
        If Ch2_Offset1 <= 0 Then
            Ch2_Offset1 = 0
            'MessageBox.Show("Ch2 Offset Limited to Minimum")
        End If

        If Ch2_Offset1 >= 1024 Then
            Ch2_Offset1 = 1023
            'MessageBox.Show("Ch2 Offset Limited to Maximum")
        End If
...
#Region "Setup Trigger Modes"
    Private Sub Set_Trigger_Mode()
        ...
        ElseIf LH_Trig_RadioButton().Checked = True Then
            TrigPolarityCmd(1) = &H0
        End If
        ...
        If Run_Button.Text <> "BUSY" Then 'Avoid problem when RUN button is pressed when Busy
            ...
            Do
                If ETS_Error <> 1 Then 'Check not in ETS with Auto
                    Setup()
                    System.Threading.Thread.Sleep(20) 'sleep 50mSec
                    mPort.DiscardInBuffer() 'Clear Input Buffer
                    'Run Command
                    mPort.Write(CaptureCmd, 0, 1)
...
#Region "Select_Mode"
    Private Sub Select_Mode()
        'Dim SetModeCmd() As Byte = {&H46, &H0} 'F 0/1/2 Ch1&Ch2 200+200 / Ch1 400/ Ch2 400
        'Read_Mode = 1  200 CH1 + 200 CH2
        'Read_Mode = 2  200 CH1 + 200 CH1
        'Read_Mode = 3  200 CH2 + 200 CH2
...
#Region "Plot"
    ...
    Private Sub Plot()
        ...
        If BothCh_Display_RadioButton.Checked Then
            ...
            For rowno = 1 To (Data_Length)
                ...
                list2.Add(x, y2)
            Next
            myPane.YAxis.Scale.Min = -12.5 / Ch1_Gain
            myPane.YAxis.Scale.Max = 12.5 / Ch1_Gain
            myPane.Y2Axis.Scale.Min = -12.5 / Ch2_Gain
            myPane.Y2Axis.Scale.Max = 12.5 / Ch2_Gain
        ...
        ElseIf CH1_Display_RadioButton.Checked() Then
            ...
            myPane.YAxis.Scale.Max = 12.5 / Ch1_Gain
            myPane.XAxis.Scale.Min = 0
            myPane.XAxis.Scale.Max = Data_Length / 2 * Multiplier
        ElseIf CH2_Display_RadioButton.Checked() Then
            ...
            For rowno = 1 To (Data_Length / 2)
                ...
                list.Add(x, y)
            Next
            myPane.YAxis.Scale.Min = -12.5 / Ch2_Gain
            myPane.YAxis.Scale.Max = 12.5 / Ch2_Gain
            myPane.XAxis.Scale.Min = 0
            myPane.XAxis.Scale.Max = Data_Length / 2 * Multiplier
        ElseIf XY_Display_RadioButton.Checked() Then
            ...
            myPane.YAxis.Scale.Min = -12.5 / Ch2_Gain
            myPane.YAxis.Scale.Max = 12.5 / Ch2_Gain
            myPane.XAxis.Scale.Min = -12.5 / Ch1_Gain
            myPane.XAxis.Scale.Max = 12.5 / Ch1_Gain
    ...
    End Sub
    Private Sub SetSize()
        Dim loc1 As New Point(50, 100) 'Rect
        Dim loc2 As New Point(100, 80) 'Sq
        'Aj_ZG1.Location = loc()
        ' Leave a small margin around the outside of the control
        'Dim size1 As New Size(Me.ClientRectangle.Width - Me.ClientRectangle.Width * 250 / 800, Me.ClientRectangle.Height - Me.ClientRectangle.Height * 140 / 600) 'Rectangle
        'Dim size2 As New Size(Me.ClientRectangle.Height - Me.ClientRectangle.Height * 150 / 600, Me.ClientRectangle.Height - Me.ClientRectangle.Height * 140 / 600) 'Square

        Dim size1 As New Size(640, 480) 'Rectangle
        Dim size2 As New Size(520, 520) 'Square
        ...
        If Sq_Display = False Then 'Choose depending on time or XY, DFT
            Aj_ZG1.Location = loc1
            Aj_ZG1.Size = size1 'Rectangular
        Else
            Aj_ZG1.Location = loc2
...
#Region "New Parts"
    ...
    Private Sub TestLEDButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
...
#Region "Read"
    ...
    Private Sub Read()
        ...
        If Read_Mode = 1 Then  'Read_Mode = 1  200 CH1 + 200 CH2
            ...
            While (mPort.BytesToRead = 0)
            End While
            mPort.Read(Ch1_dataArray, 0, 200) ' Read Serial data

            System.Threading.Thread.Sleep(50) 'sleep 30mSec

            ReadDataCmd(1) = &H2 '200 CH2
            mPort.Write(ReadDataCmd, 0, 2) 'CH2 Data
            'Sleep a bit
            System.Threading.Thread.Sleep(50) 'sleep 30mSec
            'Wait for data and read it 
            While (mPort.BytesToRead = 0)
            End While
            mPort.Read(Ch2_dataArray, 0, 200) ' Read Serial data
            '------------------------------------------------------
        ElseIf Read_Mode = 2 Then 'Read_Mode = 2  200 CH1 + 200 CH1
            ReadDataCmd(1) = &H3 'First 200 CH1
            mPort.Write(ReadDataCmd, 0, 2)
            'Sleep a bit
            System.Threading.Thread.Sleep(50) 'sleep 30mSec
            'Wait for data and read it 
            While (mPort.BytesToRead = 0)
            ...
            ReadDataCmd(1) = &H4 'Second 200 CH1
            mPort.Write(ReadDataCmd, 0, 2)
            'Sleep a bit
            System.Threading.Thread.Sleep(50) 'sleep 30mSec
            'Wait for data and read it 
            While (mPort.BytesToRead = 0)
            End While
            mPort.Read(Ch1_dataArray, 200, 200) ' Read Serial data
            '-------------------------------------------------------
        ElseIf Read_Mode = 3 Then
            ReadDataCmd(1) = &H3 'First 200 CH2
            mPort.Write(ReadDataCmd, 0, 2)
            'Sleep a bit
            System.Threading.Thread.Sleep(50) 'sleep 30mSec
            'Wait for data and read it 
            ...
            ReadDataCmd(1) = &H4 'Second 200 CH2
            mPort.Write(ReadDataCmd, 0, 2)
            'Sleep a bit
            System.Threading.Thread.Sleep(50) 'sleep 30mSec
            'Wait for data and read it 
            While (mPort.BytesToRead = 0)
            End While
            mPort.Read(Ch2_dataArray, 200, 200) ' Read Serial data
            '-------------------------------------------------------
        End If
