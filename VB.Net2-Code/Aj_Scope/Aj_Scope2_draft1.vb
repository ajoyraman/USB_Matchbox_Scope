...
#Region "SETUP OLD"
    ...
    Private Sub Setup()
        Compute_Offset_Trigger() 'sets up the Analog Command Strings
        'Write Analog values
        mPort.Write(myAnalog_Offset1, 0, 5)
        mPort.Write(myAnalog_Trig1, 0, 5)
        mPort.Write(myAnalog_Offset2, 0, 5)
        mPort.Write(myAnalog_Trig2, 0, 5)

        'Delay for PWM DAC's to settle
        System.Threading.Thread.Sleep(30) 'sleep 10mSec

        Set_Gain_Bits() 'Do this first before trigger mode setting
        Set_Trigger_Mode()
        Set_Mode() 'Sets the data length
        'Write Digital Strings
        mPort.Write(myDigital_Cmd, 0, 5)
...
#Region "Setup Gain, Offset , Trigger Level & VREF"
    Private Sub Compute_Offset_Trigger()
        ...
        End While
        System.Threading.Thread.Sleep(10) 'sleep 5mSec
...
#Region "Setup Trigger Modes"
    ...
    End Sub
#End Region
#Region " Setup MODE Normal/FFT CH1/FFT CH2"
    Private Sub Set_Mode()
        'Dim SetModeCmd() As Byte = {&H46, &H0} 'F 0/1/2 
        'Ch1 & Ch2 200 each/ Ch1 400/ Ch2 400
        If Samples_RadioButton1.Checked = True Then
            SetModeCmd(1) = &H0
            Data_Length = 200
        ElseIf Samples_RadioButton2.Checked = True Then
            SetModeCmd(1) = &H1
            Data_Length = 400
        ElseIf Samples_RadioButton3.Checked = True Then
            SetModeCmd(1) = &H2
            Data_Length = 400
        End If
...
#Region "Run"
    Private Sub Run_Button_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Run_Button.Click
        ...
        If Run_Button.Text <> "BUSY" Then 'Avoid problem when RUN button is pressed when Busy
            ...
            'Set_Sample_Rate() 'Sets the value for myRun_Cmd(2)
            '                  Header1, Multiplier & Checks ETS Valid
            'Select_Mode() 'myRun_Cmd(2) 00=Continuous, 01=Sliding, 02 = Normal

            'Check for Capture method Single, Continuous, Storage
            If Single_RadioButton.Checked = True Then
                Keep_Running = False
                Overplot = False
            ElseIf Continuous_RadioButton.Checked = True Then
                Keep_Running = True 'Till Stopped
                Overplot = False
            ElseIf OverWrite_RadioButton.Checked = True Then
                Keep_Running = False 'Till Stopped
                Overplot = True
            End If

            'Set BUSY and Issue the RUN Command
            ...
            Do
                If ETS_Error <> 1 Then
                    ...
                    'mPort.DiscardInBuffer() 'Clear Input Buffer for Ack
                    'mPort.Write(myRun_Cmd, 0, 5) ' Run command
                    '-------------------------------------------------
            ...
            'Wait for ack
            'mPort.Write(myRun_Cmd, 0, 5)
            'System.Threading.Thread.Sleep(5000) 'sleep 1000mSec
            'Run_Button.Text = "HALTED"
        End If
    End Sub
...
#Region "Read Data ---old"
    Private Sub Read_Data()
        ...
        ' Data_Length = 100, 200, 400 setup earlier
        While (mPort.BytesToRead = 0)
        End While
        ...
        mPort.DiscardInBuffer() 'Clear Input Buffer
        'Issue Read Command
        mPort.Write(myRead_Cmd, 0, 5) 'write the header
        'Sleep a bit
        System.Threading.Thread.Sleep(50) 'sleep 30mSec
...
#Region "New Parts"
    ...
    Private Sub Trig_mode_Button_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Trig_mode_Button.Click
        Set_Trigger_Mode()
        'Set_Mode()
        Select_Mode()
    End Sub

    Private Sub Gain_offset_Button_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Gain_offset_Button.Click
        Compute_Offset_Trigger()
        Set_Gain_Bits()
    End Sub
    ...
    Private Sub SetSamplingRateButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SetSamplingRateButton.Click
        Set_Sample_Rate()
...
#End Region
    ...
    Private Sub DFT_Button_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DFT_Button.Click
        doDFT()
    End Sub
#Region "Run1" 'Alternate SETUP
    Private Sub RUN1Button_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RUN1Button.Click
        'Private Sub Run1()
        'Dim BusyArray() As Byte = {&H42, &H42}
        'Dim BusyBit As Byte = &H42
        Set_Trigger_Mode()
        System.Threading.Thread.Sleep(150) 'sleep 50mSec
        Select_Mode()
        System.Threading.Thread.Sleep(150) 'sleep 50mSec
        Compute_Offset_Trigger()
        System.Threading.Thread.Sleep(150) 'sleep 50mSec
        ...
        'System.Threading.Thread.Sleep(50) 'sleep 30mSec
        '-----mPort.Write(CaptureCmd, 0, 1)
        'Look for busy complete

        'While (BusyBit <> &H62)
        'While (mPort.BytesToRead = 0)
        'End While
        'mPort.Read(BusyArray, 0, 1) ' Read Busy Bit 
        'BusyBit = BusyArray(0)
        'End While
        'Read1()
        'Plot1()
...
#End Region
    ...
    Private Sub Run2Button_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Run2Button.Click
        'mPort.Write(TrigSourceCmd, 0, 2)
        'mPort.Write(TrigPolarityCmd, 0, 2)
        mPort.DiscardInBuffer() 'Clear Input Buffer
