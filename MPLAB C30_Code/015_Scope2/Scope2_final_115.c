...
// 1o Aug 2013  Increased delay for 400 data to 200usec
// ------------------------------------------------------------
// Problems:
// Cannot read BUSY but a Busy indication sent on 232
// 
// 
// 
//-------------------------------------------------------------
...
unsigned char capture_mode,sampling_mode, error,fft;
int data_index;
 	...
 	while(1){	
		...
		if ( error==0) { 
            ...
            if (input_string[0]==73){ 		//"I" Identify
				...
				else if (input_string[1]==3){//400 samples CH1/CH2/FFT
